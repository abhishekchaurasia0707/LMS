...
export const getRazorpayKey = createAsyncThunk("/razorpay/getKey", async () => {
    try {
        const response = await axiosInstance.get('http://localhost:5000/api/v1/payments/key');
...
export const purchaseCourseBundle = createAsyncThunk("/purchaseCourse", async () => {
    try {
        const response = await axiosInstance.post('http://localhost:5000/api/v1/payments/subscribe')
        return response.data
    } catch (error) {
        toast.error(error?.response?.data?.message)
        throw error
...
export const verifyUserPayment = createAsyncThunk("/verifyPayment", async (data) => {
    try {
        ...
        const response = await axiosInstance.post('http://localhost:5000/api/v1/payments/verify', {
...
export const getPaymentsRecord = createAsyncThunk("/paymentsRecord", async () => {
    try {
        ...
        const response = await axiosInstance.get("http://localhost:5000/api/v1/payments?count=100")
...
export const cancelSubscription = createAsyncThunk("/cancel/subscribtion", async () => {
    try {
        ...
        const response = await axiosInstance.post("http://localhost:5000/api/v1/payments/unsubscribe")
        if (response.status === 200) {
            toast.dismiss();
            toast.success(response.data.message);
            return response.data;
